"""DRASTIC Sudan v11.0 - Arabic UI, Error-Free"""
...
# ===== دوال DRASTIC =====
def get_d_rating(d):
    if d < 0: raise ValueError("قيمة سالبة")
    if d <= 1.5: return 10
    if d <= 4.6: return 9
    if d <= 9.1: return 7
    if d <= 15.2: return 5
...
def get_r_rating(r):
    if r < 0: raise ValueError("قيمة سالبة")
    if r <= 50.8: return 1
    if r <= 101.6: return 3
    if r <= 177.8: return 6
    if r <= 254.0: return 8
...
def get_t_rating(t):
    if t < 0: raise ValueError("قيمة سالبة")
    if t <= 2.0: return 10
    if t <= 6.0: return 9
    if t <= 12.0: return 5
    if t <= 18.0: return 3
...
def get_c_rating(c):
    if c < 0: raise ValueError("قيمة سالبة")
    if c <= 4.074: return 1
...
def calc_travel(d, p, k, g=1.0):
    if d <= 0: raise ValueError("العمق يجب أن يكون أكبر من صفر")
    if not (0.01 < p < 0.60): raise ValueError("المسامية خارج النطاق")
    if k <= 0: raise ValueError("النفاذية يجب أن تكون أكبر من صفر")
    v = (k * g) / p
    days = d / v
    return {"days": days, "years": days / 365.25, "velocity": v}

def mitigate(idx, hdpe, treatment, monitoring):
    m = idx
    if hdpe: m *= 0.40
    if treatment: m *= 0.60
...
def calc_accuracy(preds, actuals):
    if len(preds) != len(actuals):
        raise ValueError("الطول غير متطابق")
    if len(preds) == 0:
...
def interp_acc(a):
    ...
    if a >= 80: return "جيد جدا - قابل للاعتماد"
...
def gen_report(site, coords, idx, ratings, values, travel=None):
    ...
    L.append("")
    L.append("D - العمق: " + str(values.get("depth", "غير متوفر")))
    L.append("R - التغذية: " + str(values.get("recharge", "غير متوفر")))
    L.append("A - الخزان: " + str(values.get("aquifer", "غير متوفر")))
    L.append("S - التربة: " + str(values.get("soil", "غير متوفر")))
    L.append("T - الانحدار: " + str(values.get("slope", "غير متوفر")))
    L.append("I - غير المشبعة: " + str(values.get("vadose", "غير متوفر")))
    L.append("C - النفاذية: " + str(values.get("conductivity", "غير متوفر")))
    L.append("")
    if travel:
    ...
    L.append("-" * 60)
    L.append("1. Aller et al. 1987. EPA6002-87-035")
    L.append("2. Fetter 2001. Applied Hydrogeology")
    L.append("3. US EPA 1993. EPA600R-93-174")
    L.append("4. WHO 2022. Drinking-water Quality")
    L.append("")
    L.append("-" * 60)
    L.append("القسم 7: المراجعة والاعتماد")
    L.append("-" * 60)
    L.append("هذا التقرير مولد آليا. لا يعتمد إلا بعد مراجعة")
...
with t1:
    ...
    with c2:
        ...
        vadose = st.selectbox("I - المنطقة غير المشبعة:", ["sand_gravel", "sandstone",
    ...
    try:
        ...
        b3.metric("C", C_r); b4.metric("المسامية", round(porosity, 2))
        st.markdown("---")
        c1, c2 = st.columns(2)
        c1.metric("مؤشر DRASTIC", str(idx) + " / 230")
        c2.metric("مستوى الخطورة", risk["level"])
        ...
        z3.metric("سرعة التسرب", round(travel["velocity"], 6))
    ...
    if f:
        try:
            ...
            for i, row in df.iterrows():
                try:
                    ...
                    results.append({"الموقع": row.get("name", "موقع " + str(i)),
                        "خط العرض": row.get("lat", 0), "خط الطول": row.get("lon", 0),
            ...
            if results:
                ...
                k4.metric("مواقع خطرة", len(dfr[dfr["المؤشر"] >= 140]))
...
# ===== TAB 3: الحلول (بـ radio بدل checkbox) =====
with t3:
    st.header("محاكي الحلول الهندسية")
    if "ci" not in st.session_state:
        st.warning("الرجاء اختيار موقع من التبويب الأول")
    else:
        base = st.session_state["ci"]
        st.info("الموقع: " + st.session_state["cs"] + " | المؤشر: " + str(base))
        
        st.markdown("---")
        st.subheader("اختر الحلول (يمكن اختيار أكثر من واحد):")
        
        c1, c2, c3 = st.columns(3)
        
        with c1:
            hdpe_choice = st.radio(
                "بطانة HDPE",
                options=["لا", "نعم"],
                index=0,
                key="r_hdpe",
        ...
        with c3:
            ...
            monitoring = (mon_choice == "نعم")
        
        if hdpe or treatment or monitoring:
            r = mitigate(base, hdpe, treatment, monitoring)
            st.markdown("---")
            st.subheader("النتائج")
            a, b, c = st.columns(3)
            a.metric("قبل التخفيض", base)
            b.metric("بعد التخفيض", r["mitigated_index"])
            c.metric("نسبة التخفيض", str(r["reduction_pct"]) + "%")
            ...
            st.info("اختر حلا واحدا على الأقل")
...
with t4:
    ...
    if "ci" not in st.session_state:
        ...
        if "rep" in st.session_state:
            ...
            with cc2:
                st.download_button("تحميل HTML (موصى به)",
...
with t6:
    ...
    st.markdown("ارفع ملف CSV يحتوي على: name, drastic_index, actual_status")
    st.markdown("actual_status: 1 = ملوث، 0 = نظيف")
    sample = pd.DataFrame({
        "name": ["موقع 1", "موقع 2", "موقع 3", "موقع 4"],
        "drastic_index": [150, 80, 130, 165],
        "actual_status": [1, 0, 0, 1]})
    st.download_button("تحميل نموذج",
        data=sample.to_csv(index=False).encode("utf-8-sig"),
        file_name="accuracy_template.csv", mime="text/csv")
    f2 = st.file_uploader("ارفع الملف:", type=["csv", "xlsx"], key="acc")
    if f2:
        try:
            ...
            if "drastic_index" not in df.columns or "actual_status" not in df.columns:
                st.error("يجب توفر عمودين: drastic_index و actual_status")
            else:
                ...
                rep_lines = [
                    ...
                    "",
                    "المقاييس:",
                    "  الدقة: " + str(m["accuracy"]) + " percent",
                    "  الحساسية: " + str(m["recall"]) + " percent",
                    "  الدقة الموجبة: " + str(m["precision"]) + " percent",
                    "  التحديد: " + str(m["specificity"]) + " percent",
                    "  F1: " + str(m["f1"]) + " percent",
                    "  Kappa: " + str(m["kappa"]),
                    "",
                    "التفسير:",
