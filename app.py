...
with tab1:
    ...
    with col_input:
        ...
        if coord_mode == "جغرافي (Lat/Lon) والبحث":
            ...
            try:
                proj_utm = pyproj.Proj(proj='utm', zone=utm_zone, ellps='WGS84', datum='WGS84')
                lon_val, lat_val = proj_utm(utm_easting, utm_northing, inverse=True)
        ...
        # حساب مؤشر الخطر وفق معيار DRASTIC العالمي
    ...
    with col_display:
        ...
        # ==========================================
        # محرك التقرير الذكي المزدوج (Cloud AI + Hybrid Fallback Engine)
        # ==========================================
        st.markdown("---")
        ...
        if st.button("✨ توليد تقرير فني فوري", type="primary", use_container_width=True):
            ...
            # 1. محاولة الاتصال بـ Gemini API
            ...
            # 2. التوليد عبر المحرك المحلي الصلب عند تعذر السحابة (Fallback Engine)
            if not ai_generated:
