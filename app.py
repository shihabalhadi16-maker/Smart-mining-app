...
# ==========================================
preset_locations = {
    "سوق طواحين أبو حمد (نهر النيل)": {"coords": (19.5333, 33.3167), "depth": 15, "soil": "تربة رملية هشّة (نفاذية عالية)", "cyanide": 0.45, "mercury": 0.08, "ph": 7.4},
    "عطبرة - النيل الكبرى (نهر النيل)": {"coords": (17.6833, 33.9833), "depth": 8, "soil": "تربة رملية هشّة (نفاذية عالية)", "cyanide": 0.80, "mercury": 0.12, "ph": 7.2},
    "سوق العبيدية (نهر النيل)": {"coords": (18.1234, 33.9876), "depth": 10, "soil": "تربة رملية هشّة (نفاذية عالية)", "cyanide": 0.65, "mercury": 0.09, "ph": 7.6},
    "مناجم بربر (نهر النيل)": {"coords": (18.0167, 33.9833), "depth": 12, "soil": "تربة طمية مختلطة (نفاذية متوسطة)", "cyanide": 0.30, "mercury": 0.04, "ph": 7.8},
    "وادي العشاري / قبقبة (الشمالية)": {"coords": (21.8000, 34.5000), "depth": 60, "soil": "تربة صخرية صلبة (نفاذية منخفضة)", "cyanide": 0.10, "mercury": 0.02, "ph": 8.0},
    "وادي حلفا - كرمة (الشمالية)": {"coords": (21.7950, 31.3700), "depth": 25, "soil": "تربة رملية هشّة (نفاذية عالية)", "cyanide": 0.50, "mercury": 0.06, "ph": 7.5},
    "مناجم أرياب (البحر الأحمر)": {"coords": (18.3333, 36.3500), "depth": 45, "soil": "تربة صخرية صلبة (نفاذية منخفضة)", "cyanide": 0.05, "mercury": 0.01, "ph": 8.1},
    "سوق دلقو (الشمالية)": {"coords": (20.3000, 30.5500), "depth": 35, "soil": "تربة صخرية صلبة (نفاذية منخفضة)", "cyanide": 0.15, "mercury": 0.03, "ph": 7.9},
    "تلودي / الليري (جبال النوبة)": {"coords": (10.6333, 30.1167), "depth": 18, "soil": "تربة طمية مختلطة (نفاذية متوسطة)", "cyanide": 0.70, "mercury": 0.15, "ph": 6.8},
    "كادوقلي (جنوب كردفان)": {"coords": (11.0167, 29.7167), "depth": 20, "soil": "تربة طمية مختلطة (نفاذية متوسطة)", "cyanide": 0.20, "mercury": 0.05, "ph": 7.1}
}
...
# ==========================================
st.sidebar.image("https://upload.wikimedia.org/wikipedia/en/thumb/8/82/University_of_Khartoum_logo.png/220px-University_of_Khartoum_logo.png", width=110)
st.sidebar.markdown("### ⛏️ جامعة الخرطوم\n**قسم هندسة التعدين**")

st.sidebar.markdown("---")
st.sidebar.header("🌍 البحث والتحويل الإحداثي")

coord_mode = st.sidebar.radio("نظام التحديد:", ["اسم المنطقة / Lat-Lon", "إحداثيات متريّة (UTM Zone 36N/37N)"])

if coord_mode == "اسم المنطقة / Lat-Lon":
    st.sidebar.selectbox("مواقع مسجلة جاهزة:", list(preset_locations.keys()), key="preset_select", on_change=on_preset_change)
    custom_search = st.sidebar.text_input("أو ابحث عن موقع:", placeholder="مثال: Port Sudan أو العبيدية")
    if st.sidebar.button("بحث 🧭"):
        q = custom_search.strip()
        if q:
            geolocator = Nominatim(user_agent=f"uofk_mining_app_{random.randint(1000, 9999)}")
            try:
                loc = geolocator.geocode(f"{q}, Sudan", timeout=8) or geolocator.geocode(q, timeout=8)
                if loc:
    ...
    if st.sidebar.button("تحويل الإحداثيات 🔄"):
        try:
            ...
            st.session_state.selected_site_name = f"UTM ({utm_easting}, {utm_northing})"
            st.rerun()
...
river_dist = st.sidebar.slider("البعد عن أقرب مجرى مائي (متر):", 50, 5000, 300, step=50)
soil = st.sidebar.selectbox("نوع التربة السطحية:", ["تربة رملية هشّة (نفاذية عالية)", "تربة طمية مختلطة (نفاذية متوسطة)", "تربة صخرية صلبة (نفاذية منخفضة)"])

col_sb1, col_sb2 = st.sidebar.columns(2)
with col_sb1:
    cyanide = st.number_input("السيانيد (mg/L):", min_value=0.01, max_value=5.00, value=0.45, step=0.05)
with col_sb2:
    mercury = st.number_input("الزئبق (mg/L):", min_value=0.000, max_value=1.000, value=0.080, step=0.005)
...
badge_color = "#d8f3dc" if risk_status == "LOW" else ("#ffe8d6" if risk_status == "MEDIUM" else "#ffccd5")
badge_text_color = "#1b4332" if risk_status == "LOW" else ("#a71e1c" if risk_status == "HIGH" else "#9c6644")
...
with tab1:
    ...
    with k1:
        st.metric("🧪 Cyanide Concentration", f"{cyanide} mg/L", delta="Safe Limit: 0.20 mg/L", delta_color="normal" if cyanide <= 0.20 else "inverse")
    with k2:
        st.metric("💧 pH Level", f"{ph_level}", delta="Optimal: 6.5 - 8.5", delta_color="normal" if 6.5 <= ph_level <= 8.5 else "inverse")
    with k3:
        st.metric("⚗️ Mercury (Hg)", f"{mercury} mg/L", delta="Safe Limit: 0.006 mg/L", delta_color="normal" if mercury <= 0.006 else "inverse")
    with k4:
        st.metric("⏳ زمن الوصول للجوف", f"{years} سنة", delta=f"العمق: {depth}m")
    ...
    marker_color = "green" if risk_score < 40 else ("orange" if risk_score < 70 else "red")
    
    folium.Marker(
        [st.session_state.lat, st.session_state.lon],
        popup=f"<b>الموقع:</b> {st.session_state.selected_site_name}<br><b>درجة الخطر:</b> {risk_score}%",
    ...
    if st.button("📄 توليد تقرير استشاري فني فوري (AI Report)", type="primary"):
            ...
            for model_name in ['gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-2.0-flash']:
                ...
                try:
                    ...
                    for chunk in response:
                        ...
                        report_container.markdown(f"<div class='report-box'>{full_text}▌</div>", unsafe_allow_html=True)
                    report_container.markdown(f"<div class='report-box'>{full_text}</div>", unsafe_allow_html=True)
                    ai_generated = True
                except Exception:
                    continue
        ...
        if not ai_generated:
            ...
            **الموقع:** {st.session_state.selected_site_name} | **درجة الخطر:** {risk_score}%
            ...
            1. **التقييم الجيوهيدرولوجي والمخاطر:**
               - تقييم نموذج DRASTIC يشير إلى خطر درجة ({risk_status}).
               - زمن الوصول المتوقع للملوثات إلى المياه الجوفية بعمق {depth}m هو **{years} سنة**.

            2. **الأثر البيئي والصحي المباشر:**
               - السيانيد: **{cyanide} mg/L** | الزئبق: **{mercury} mg/L** | pH: **{ph_level}**.
               - قرب الموقع ({river_dist}m) من المجاري المائية يتطلب احتياطات عزل إضافية.

            3. **التوصيات الهندسية الميدانية:**
               - تركيب بطانات عازلة بتركيب HDPE Liner عالي الكثافة.
               - حفر آبار مراقبة اختبارية لمتابعة حركة المياه الجوفية.
            """
            report_container.markdown(f"<div class='report-box'>{fallback_text}</div>", unsafe_allow_html=True)
...
with tab2:
    ...
    uploaded_file = st.file_uploader("اختر ملف CSV أو Excel يحتوي على الإحداثيات والملوثات:", type=["xlsx", "csv"])
        ...
        try:
            ...
            with c_heat:
                if 'Latitude' in df_bulk.columns and 'Longitude' in df_bulk.columns:
                    ...
                    heat_val = 'Cyanide' if 'Cyanide' in df_bulk.columns else df_bulk.columns[2]
                    heat_data = [[row['Latitude'], row['Longitude'], row[heat_val]] for index, row in df_bulk.iterrows()]
                    HeatMap(heat_data, radius=15).add_to(m_heat)
                    st_folium(m_heat, width="100%", height=380, key="bulk_heat_map")
                else:
                    st.error("الملف يحتاج إلى ألوح تحوي أعمدة 'Latitude' و 'Longitude'")
        except Exception as e:
...
with tab3:
    ...
    with col_sc2:
        ...
        treatment = st.checkbox("تطبيق وحدة معالجة وتفكيك السيانيد بالكيميائيات (Cyanide Destruction)")
        mercury_stop = st.checkbox("استبدال الزئبق بتقنيات إعادة التدوير والخضخضة العازلة")
